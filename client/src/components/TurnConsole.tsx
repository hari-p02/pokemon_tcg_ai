...
const TurnConsole: React.FC<TurnConsoleProps> = ({ onGameStateUpdated }) => {
  ...
  const handlePlayerTurn = async (playerNumber: 1 | 2) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/player${playerNumber}/turn`);
  ...
  return (
    <VStack spacing={4} p={4} border="1px solid" borderColor="gray.200" borderRadius="md" bg="white" width="100%">
      <Text fontSize="lg" fontWeight="bold" alignSelf="flex-start">
        Turn Console {isLoading && <Text as="span" color="orange.500">(Processing...)</Text>}
      </Text>
      ...
      >
        {outputLines.length === 0 ? (
          <Text color="gray.400">No output yet. Start a turn to see results.</Text>
        ) : (
