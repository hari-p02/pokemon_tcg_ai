...
def create_pokemon_tcg_graph():
    ...
    def referee_node(state: ConversationState) -> ConversationState:
        ...
        # Get response from referee agent
        try:
            # Use regular invoke instead of streaming to avoid JSON parsing issues
            ...
            if "ILLEGAL" in response_text:
                state["decision_is_legal"] = False
                state["decision_explanation"] = response_text
                ...
                try:
                    ...
                    if start_index == -1:
                        ...
                        json_data = response_text[start_index+7:end_index]
                    ...
                    state["decision_is_legal"] = False
                    state["decision_explanation"] = f"Error processing action: {response_text}"
                    state["updated_game_state"] = state["game_state"]
